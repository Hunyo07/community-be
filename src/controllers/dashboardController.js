...
export const getDashboard = async (req, res, next) => {
  try {
    if (req.user?.role === ROLES.RESIDENT) {
      ...
      return res.json({
        data: {
          dashboardMetrics: [
            {
              ...
              value: numberFormat.format(
                Number(serviceCounts.activeServices || 0),
              ),
              trend: "Community programs",
              tone: "success",
            },
            {
              label: "My Requests",
              value: numberFormat.format(
                Number(requestCounts.totalRequests || 0),
              ),
              trend: `${numberFormat.format(Number(requestCounts.openRequests || 0))} active`,
              tone: "info",
            },
            {
              label: "Pending Updates",
              value: numberFormat.format(
                Number(requestCounts.openRequests || 0) +
                  (resident?.verificationStatus === "Pending" ? 1 : 0),
              ),
              trend: resident?.verificationStatus || "Pending",
              tone:
            ...
            {
              label: "Barangay",
              value: resident?.barangay || req.user.barangay || "Assigned",
              trend: "Resident profile",
              tone: "primary",
            },
            {
              label: "Total Services",
              value: numberFormat.format(
                Number(serviceCounts.totalServices || 0),
              ),
          ...
          barangayPerformance: [
            {
              label: resident?.barangay || req.user.barangay || "Your Barangay",
              value: resident?.verificationStatus || "Pending",
              status: resident?.verificationStatus || "Pending",
            },
          ],
          recentActivity: [
            {
              title: "Resident dashboard ready",
              description: "Track requests and account updates here.",
              time: "Today",
            },
          ],
          residentSummary: [
            ...
            },
            {
              ...
              value: resident?.accountStatus || "Inactive",
            },
            {
              label: "Barangay",
              value: resident?.barangay || req.user.barangay || "Assigned",
    ...
    const pendingRequests =
      Number(requestCounts.openRequests || 0) +
      Number(residentCounts.pendingResidents || 0);
    const barangays = Number(residentCounts.barangays || 0);

    const dashboardMetrics = [
      {
        label: "Residents",
        value: numberFormat.format(totalResidents),
        trend: `${numberFormat.format(Number(residentCounts.verifiedResidents || 0))} verified`,
        tone: "info",
      },
      {
        label: "Services",
        value: numberFormat.format(totalServices),
        trend: `${numberFormat.format(activeServices)} active`,
        tone: "success",
      },
      {
        label: "Pending Requests",
        value: numberFormat.format(pendingRequests),
        trend: "Needs review",
        tone: "warning",
      },
      {
        label: "Barangays",
        value: numberFormat.format(barangays),
        trend: "With registered residents",
        tone: "primary",
      },
      {
        label: "Areas",
        value: numberFormat.format(Math.max(barangays, 1)),
        trend: "Coverage zones",
        tone: "info",
      },
    ];

    const maxBeneficiaries = Math.max(
      ...serviceUtilizationRows.map((row) => Number(row.beneficiaries)),
      1,
    );
    ...
    const barangayPerformance = barangayRows.map((row) => {
      ...
      const percent =
        residents > 0 ? Math.round((verified / residents) * 100) : 0;
      return {
        label: row.barangay,
        value: `${percent}%`,
        status:
          percent >= 80 ? "Verified" : percent >= 50 ? "Active" : "Pending",
      };
    });
    ...
    const residentSummary = [
      ...
      },
      {
        ...
        value: numberFormat.format(
          Number(residentCounts.verifiedResidents || 0),
        ),
      },
      {
        label: "Active Accounts",
        value: numberFormat.format(Number(residentCounts.activeResidents || 0)),
      },
      {
