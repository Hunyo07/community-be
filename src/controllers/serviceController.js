...
// Maps a services table row into the shape used by the frontend API.
const mapService = (service) => ({
  ...
  targetBeneficiaries: Number(
    service.target_residents ?? service.target_beneficiaries ?? 0,
  ),
  requests: service.pending_requests,
  remainingBeneficiaries: Math.max(
    Number(service.target_residents ?? service.target_beneficiaries ?? 0) -
      Number(service.served_count ?? service.beneficiaries ?? 0),
    0,
  ),
...
const allowedBeneficiaryStatuses = [
  "Pending",
  "Served",
  "Skipped",
  "Not Eligible",
...
// Escapes a single CSV cell so commas and quotes do not break the file.
...
// Builds a full CSV string from column definitions and row objects.
...
// Sends a CSV download response with a UTF-8 BOM for spreadsheet apps.
...
const getActiveServiceCategories = async () => {
  const [rows] = await pool.query(
    `SELECT name FROM service_categories WHERE status = 'Active'`,
  );
...
const getOfficeById = async (id) => {
  ...
  const [rows] = await pool.execute(
    "SELECT id, barangay, status FROM offices WHERE id = ?",
    [id],
  );
  return rows[0] || null;
};

// Loads one service by id using the shared select query, or returns null.
...
const assertServiceAccess = (req, service) => {
  ...
  const sameOffice =
    !service.officeId || service.officeId === req.user.officeId;
  const canViewAllOffices = req.user.permissions?.includes(
    "services:view_all_offices",
  );
...
const assertWritableServiceScope = (req, payload) => {
  ...
  const sameOffice =
    !payload.officeId || payload.officeId === req.user.officeId;
  const canViewAllOffices = req.user.permissions?.includes(
    "services:view_all_offices",
  );
  const canAssignOffice = req.user.permissions?.includes(
    "services:assign_office",
  );

  if (!sameBarangay) {
    return Object.assign(
      new Error(
        "Barangay staff can only manage services in their assigned barangay",
      ),
      { statusCode: 403 },
    );
  }
  ...
  if (!sameOffice && !canViewAllOffices && !canAssignOffice) {
    return Object.assign(
      new Error(
        "Barangay staff can only manage services in their assigned office",
      ),
      { statusCode: 403 },
    );
  }
...
// Forces or validates office_id for barangay staff based on their permissions.
const applyStaffOfficeScope = async (req, payload) => {
  ...
  if (!office || office.status !== "Active") {
    throw Object.assign(new Error("Selected office is not available"), {
      statusCode: 400,
    });
  }
...
// Returns services filtered by the caller's role, barangay, and query params.
const getScopedServices = async (req) => {
  ...
  if (req.user?.role === "barangay_staff") {
    ...
    if (
      !req.user.permissions?.includes("services:view_all_offices") &&
      !req.user.permissions?.includes("services:view_all_barangays")
    ) {
  ...
  if (req.user?.role === "resident") {
    filters.push(
      `(services.barangay = ? OR services.visibility IN ('all_barangays', 'public'))`,
    );
    values.push(req.user.barangay || "");
  ...
  if (search) {
    filters.push(
      "(services.name LIKE ? OR services.category LIKE ? OR services.barangay LIKE ?)",
    );
  ...
  const scopedWhereClause = filters.length
  ...
  if (req.query.coverage) {
    services = services.filter(
      (service) =>
        (req.query.coverage === "not_started" &&
          Number(service.beneficiaries || 0) === 0) ||
        (req.query.coverage === "in_progress" &&
          Number(service.beneficiaryProgress || 0) > 0 &&
          Number(service.beneficiaryProgress || 0) < 100) ||
        (req.query.coverage === "complete" &&
          Number(service.beneficiaryProgress || 0) >= 100),
    );
  }
...
// Validates and fills defaults for create/update service request bodies.
const normalizeServicePayload = async (body, existing = {}, defaults = {}) => {
  const payload = {
    ...
    startDate:
      body.startDate === ""
        ? null
        : (body.startDate ?? existing.startDate ?? null),
    endDate:
      body.endDate === "" ? null : (body.endDate ?? existing.endDate ?? null),
    remarks: body.remarks ?? existing.remarks ?? "",
    targetBeneficiaries:
      body.targetBeneficiaries ?? existing.targetBeneficiaries ?? 0,
  ...
  if (
    ...
    !payload.category ||
    !payload.barangay ||
    payload.barangay === "All Barangays"
  ) {
    throw Object.assign(
      new Error("Name, category, and assigned barangay are required"),
      { statusCode: 400 },
    );
  }
  ...
  if (!allowedCategories.includes(payload.category)) {
    throw Object.assign(new Error("Invalid service category"), {
      statusCode: 400,
    });
  }

  if (!allowedStatuses.includes(payload.status)) {
    throw Object.assign(new Error("Invalid service status"), {
      statusCode: 400,
    });
  }

  if (!allowedVisibility.includes(payload.visibility)) {
    throw Object.assign(new Error("Invalid service visibility"), {
      statusCode: 400,
    });
  }
  ...
  if (payload.targetBeneficiaries < 0 || payload.pendingRequests < 0) {
    throw Object.assign(
      new Error("Beneficiary counts must be zero or greater"),
      { statusCode: 400 },
    );
  }
...
// Lists services the current user is allowed to see.
...
// Lists publicly visible services for the cross-barangay directory view.
export const getServiceDirectory = async (req, res, next) => {
  try {
    ...
    if (search) {
      filters.push(
        "(services.name LIKE ? OR services.category LIKE ? OR services.barangay LIKE ? OR offices.name LIKE ?)",
      );
    ...
    if (
      ...
      allowedVisibility.includes(req.query.visibility)
    ) {
...
// Creates a new community service and broadcasts a realtime update.
export const createService = async (req, res, next) => {
  try {
    const defaultVisibility = await getSettingValue(
      "default_service_visibility",
      "own_barangay",
    );
    let payload = await normalizeServicePayload(
      req.body,
      {},
      {
...
// Updates an existing service after access and payload checks.
...
// Loads a service and throws if the caller cannot access it.
...
// Maps a checklist query row into the beneficiary list item the UI shows.
...
// Builds the resident checklist for a service, with optional search/filter query params.
const getServiceChecklistRows = async (service, filters = {}) => {
  ...
  if (filters.status) {
    mapped = mapped.filter(
      (resident) => resident.beneficiaryStatus === filters.status,
    );
  }
...
// Computes served, pending, and completion totals from the checklist rows.
const getServiceSummary = async (service) => {
  ...
  const served = rows.filter(
    (row) => row.beneficiaryStatus === "Served",
  ).length;
  const pending = rows.filter(
    (row) => row.beneficiaryStatus === "Pending",
  ).length;
  const skipped = rows.filter(
    (row) => row.beneficiaryStatus === "Skipped",
  ).length;
  const notEligible = rows.filter(
    (row) => row.beneficiaryStatus === "Not Eligible",
  ).length;
  ...
  return {
    ...
    skipped,
    notEligible,
    completionRate: totalResidents
      ? Math.round((served / totalResidents) * 100)
      : 0,
  };
};
...
// Returns one service with its beneficiary summary attached.
...
// Changes a service's Active/Inactive/Completed status.
export const changeServiceStatus = async (req, res, next) => {
  try {
    ...
    await pool.execute("UPDATE services SET status = ? WHERE id = ?", [
      status,
      service.rawId,
    ]);
    ...
    await logAudit({
      ...
      entityType: "services",
      entityId: service.rawId,
      details: { status },
    });
    emitRealtimeEvent("services:changed", {
...
// Returns the full resident checklist for a service.
...
// Marks or updates one resident's beneficiary status for a service.
export const updateServiceBeneficiary = async (req, res, next) => {
  try {
    ...
    const [residentRows] = await pool.execute(
      "SELECT id, barangay FROM resident_accounts WHERE id = ?",
      [residentId],
    );
    ...
    if (residentRows[0].barangay !== service.barangay) {
      return res
        .status(400)
        .json({ message: "Resident does not belong to this service barangay" });
    }
    ...
    await logAudit({
      ...
      entityId: residentId,
      details: { serviceId: service.rawId, status, remarks },
    });
    emitRealtimeEvent("services:changed", {
    ...
    const rows = await getServiceChecklistRows(service, {});
    return res.json({
      data: rows.find((row) => String(row.residentId) === String(residentId)),
    });
  } catch (error) {
    return next(error);
  }
...
// Removes a beneficiary record so the resident shows as not served again.
export const resetServiceBeneficiary = async (req, res, next) => {
  try {
    ...
    await pool.execute(
      ...
      [service.rawId, req.params.residentId],
    );
    await logAudit({
      user: req.user,
      ...
      details: { serviceId: service.rawId },
    });
    emitRealtimeEvent("services:changed", {
      action: "beneficiary-reset",
      serviceId: service.rawId,
      residentId: req.params.residentId,
    });
    emitRealtimeEvent("dashboard:changed", {
      reason: "service-beneficiary-reset",
    });
    return res.json({
      data: {
...
// Lists only residents already marked Served for a service.
...
// Lists residents in the barangay who have not been marked Served yet.
export const getNotYetServedResidents = async (req, res, next) => {
  try {
    const service = await getServiceForAction(req, req.params.id);
    const rows = await getServiceChecklistRows(service, req.query);
    return res.json({
      data: rows.filter((row) => row.beneficiaryStatus !== "Served"),
    });
  } catch (error) {
    return next(error);
  }
...
// Returns aggregate served/pending counts for one service.
...
// Downloads a CSV summary of all services visible to the caller.
...
// Downloads the full resident checklist for one service as CSV.
...
// Downloads only Served beneficiaries for one service as CSV.
export const exportServiceBeneficiariesCsv = async (req, res, next) => {
  try {
    ...
    await logAudit({
      user: req.user,
      action: "services.export_beneficiaries",
      entityType: "services",
      entityId: service.rawId,
      details: req.query,
    });
...
// Downloads residents not yet served for one service as CSV.
export const exportServiceNotYetServedCsv = async (req, res, next) => {
  try {
    const service = await getServiceForAction(req, req.params.id);
    const rows = await getServiceChecklistRows(service, req.query);
    const notYetServed = rows.filter(
      (row) => row.beneficiaryStatus !== "Served",
    );
    await logAudit({
      user: req.user,
      action: "services.export_not_yet_served",
      entityType: "services",
      entityId: service.rawId,
      details: req.query,
    });
