...
const allowedVerificationStatuses = ['Pending', 'Needs Correction', 'Verified', 'Rejected'];
const allowedAccountStatuses = ['Active', 'Inactive'];
const allowedGenders = ['Female', 'Male', 'Unspecified'];

const canViewAllResidents = (user) =>
  user?.role !== 'barangay_staff' || user.permissions?.includes(PERMISSIONS.RESIDENTS_VIEW_ALL);
...
const normalizeResidentPayload = (body, existing = {}) => {
  const payload = {
    ...
    birthDate: body.birthDate === '' ? null : body.birthDate ?? existing.birthDate ?? null,
    ...
    verificationStatus: body.verificationStatus ?? body.status ?? existing.verificationStatus ?? existing.status ?? 'Pending',
    accountStatus: body.accountStatus ?? existing.accountStatus ?? 'Inactive',
    password: body.password || ''
  };
  payload.age = payload.birthDate ? calculateAge(payload.birthDate) : body.age === '' || body.age === undefined ? existing.age ?? null : Number(body.age);

  if (!payload.firstName || !payload.lastName || !payload.email || !payload.barangay || !payload.birthDate) {
    throw Object.assign(new Error('First name, last name, email, barangay, and birthdate are required'), { statusCode: 400 });
  }

  if (payload.age === null || payload.age < 0 || payload.age > 120) {
    throw Object.assign(new Error('Birthdate must produce a valid resident age'), { statusCode: 400 });
  }

  if (payload.age !== null && (!Number.isInteger(payload.age) || payload.age < 0)) {
    throw Object.assign(new Error('Age must be a valid number'), { statusCode: 400 });
  }

  if (!allowedGenders.includes(payload.gender)) {
    throw Object.assign(new Error('Invalid resident gender'), { statusCode: 400 });
  }

  if (!allowedVerificationStatuses.includes(payload.verificationStatus)) {
    throw Object.assign(new Error('Invalid resident verification status'), { statusCode: 400 });
  }

  if (!allowedAccountStatuses.includes(payload.accountStatus)) {
    throw Object.assign(new Error('Invalid resident account status'), { statusCode: 400 });
  }
...
export const createResident = async (req, res, next) => {
  try {
    const payload = normalizeResidentPayload(applyResidentWriteScope(req.body, req.user));
    ...
    const [result] = await pool.execute(
      ...
      [
        ...
        payload.verificationStatus === 'Verified' ? payload.accountStatus : payload.verificationStatus
      ]
    );
    ...
    await logAudit({
      ...
      details: { ...payload, password: payload.password ? 'provided' : 'default', hasSelfieId: Boolean(req.file) }
    });
    emitRealtimeEvent('residents:changed', { action: 'created', data: resident });
    emitRealtimeEvent('dashboard:changed', { reason: 'resident-created' });

    return res.status(201).json({ data: resident });
  } catch (error) {
    if (error.code === 'ER_DUP_ENTRY') return res.status(409).json({ message: 'A resident account already exists for this email' });
    return next(error);
  }
};
...
export const updateResident = async (req, res, next) => {
  try {
    ...
    if (!ensureResidentAccess(existing, req.user)) {
      return res.status(403).json({ message: 'You can only manage residents in your assigned barangay' });
    }

    const payload = applyResidentWriteScope(normalizeResidentPayload(req.body, existing), req.user);
    const values = [
      payload.firstName,
      payload.lastName,
      payload.email,
    ...
    await logAudit({
      ...
      details: { ...payload, password: payload.password ? 'changed' : 'unchanged', selfieIdChanged: Boolean(req.file) }
    });
    emitRealtimeEvent('residents:changed', { action: 'updated', data: resident });
...
export const getMyResidentProfile = async (req, res, next) => {
  try {
    if (req.user?.accountType !== 'resident') {
      return res.status(403).json({ message: 'Only residents can access this profile' });
    }
...
export const updateMyResidentProfile = async (req, res, next) => {
  try {
    if (req.user?.accountType !== 'resident') {
      return res.status(403).json({ message: 'Only residents can update this profile' });
    }

    const existing = await getResidentById(req.user.id);
    if (!existing) return res.status(404).json({ message: 'Resident profile not found' });
    ...
    await pool.execute(
      `UPDATE resident_accounts
       ...
       WHERE id = ?`,
      [payload.firstName, payload.lastName, payload.contactNumber, payload.purokSitio, payload.birthDate, payload.age, payload.gender, req.user.id]
    );
    ...
    emitRealtimeEvent('residents:changed', { action: 'profile-updated', data: resident });
...
export const updateResidentStatus = async (req, res, next) => {
  try {
    ...
    const requestedVerificationStatus = verificationStatus || (allowedVerificationStatuses.includes(status) ? status : null);
    const requestedAccountStatus = accountStatus || (allowedAccountStatuses.includes(status) ? status : null);

    if (!requestedVerificationStatus && !requestedAccountStatus) {
      return res.status(400).json({ message: 'Invalid resident status update' });
    }

    const existing = await getResidentById(req.params.id);
    if (!existing) return res.status(404).json({ message: 'Resident not found' });
    if (!ensureResidentAccess(existing, req.user)) {
      return res.status(403).json({ message: 'You can only verify residents in your assigned barangay' });
    }

    const nextVerificationStatus = requestedVerificationStatus || existing.verificationStatus;
    ...
    if (!allowedVerificationStatuses.includes(nextVerificationStatus)) {
      return res.status(400).json({ message: 'Invalid resident verification status' });
    }

    if (!allowedAccountStatuses.includes(nextAccountStatus)) {
      return res.status(400).json({ message: 'Invalid resident account status' });
    }
    ...
    if (requestedAccountStatus && nextVerificationStatus !== 'Verified') {
      return res.status(400).json({ message: 'Only verified residents can be activated or deactivated' });
    }

    const legacyStatus = nextVerificationStatus === 'Verified' ? nextAccountStatus : nextVerificationStatus;
    const [result] = await pool.execute('UPDATE resident_accounts SET verification_status = ?, account_status = ?, status = ? WHERE id = ?', [
    ...
    const resident = await getResidentById(req.params.id);
    const shouldEmailResident = Boolean(requestedVerificationStatus || requestedAccountStatus);
    ...
    if (shouldEmailResident) {
      sendAccountStatusEmail({
        ...
        console.error(`Failed to send resident status email to ${resident.email}: ${error.message}`);
      });
    }
    ...
    if (shouldEmailResident) {
      ...
      emitRealtimeEvent('notifications:changed', { action: 'created', userId: resident.rawId, userRole: 'resident' });
    }
    ...
    await logAudit({
      ...
      entityType: 'resident_accounts',
      entityId: req.params.id,
      details: { verificationStatus: nextVerificationStatus, accountStatus: nextAccountStatus, reason }
    });
    emitRealtimeEvent('residents:changed', { action: 'status-updated', data: resident });
    emitRealtimeEvent('dashboard:changed', { reason: 'resident-status-updated' });
...
export const getResidentSelfieId = async (req, res, next) => {
  try {
    const [rows] = await pool.execute('SELECT barangay, selfie_id_image FROM resident_accounts WHERE id = ?', [req.params.id]);

    if (rows.length === 0) {
      return res.status(404).json({ message: 'Resident not found' });
    }

    if (!canViewAllResidents(req.user) && rows[0].barangay !== req.user?.barangay) {
      return res.status(403).json({ message: 'You can only view resident documents in your assigned barangay' });
    }

    const imagePath = rows[0].selfie_id_image;
    if (!imagePath) {
      return res.status(404).json({ message: 'Resident has no uploaded selfie with ID' });
    }

    const resolvedPath = path.isAbsolute(imagePath) ? imagePath : path.resolve(imagePath);
    if (!fs.existsSync(resolvedPath)) {
      return res.status(404).json({ message: 'Uploaded selfie with ID file was not found on disk' });
    }
